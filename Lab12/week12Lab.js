...
const dataBase = 'http://localhost:3000/studentRoster';
 ...
 *
 * ↓ YOUR CODE HERE ↓ */
// let urlJquery = 'https://api.jquery.com/jquery.post/';
...
function deleteUser(id) {
  $.ajax({
    url: dataBase + `/${id}`,// url + the template literal targets the id 
 ...
 * ↓ YOUR CODE HERE ↓ */
function updateUser() {
  let id = $('#student-name').val()

  $.ajax(`${dataBase}/${id}`, {
    method: 'PUT',
    contentType: 'application/json',
    data: JSON.stringify({
      fullName: $('#student-name').val(),
      researchAssignment: $('#student-assignment').val()
    }),
    success: function (response) {
      $('#feedback').text('User updated successfully!').css('color', 'green');
    },
    error: function (xhr, status, error) {
      $('#feedback').text(`Error updating user: ${error}`).css('color', 'red');
    }
  });
}

$('#update-btn').click(updateUser);
